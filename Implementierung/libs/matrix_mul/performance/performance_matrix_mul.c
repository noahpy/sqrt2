...
void generate_testset(unsigned int seed, size_t n, size_t stepsize, struct bignum array[][3]) {
    ...
    struct cmp_matrix2x2 current = {bignumOfInt(0), bignumOfInt(1), bignumOfInt(2)};
    struct cmp_matrix2x2 old;
    for (size_t i = 0; i < n / stepsize; i++) {
        while (current.x.size < (i * stepsize)) {
            old = current;
            current = mulCmpMatrix2x2(base, current, multiplicationBignum, additionBignum);
            freeCmp2x2(old);
        }
        array[i][0] = current.xm1;
        array[i][1] = current.x;
        array[i][2] = current.xp1;
        current = mulCmpMatrix2x2(base, current, multiplicationBignum, additionBignum);
    }
    freeCmp2x2(base);
}
...
int main () {
    ...
    size_t stepsize = 100;
    struct bignum tests[n/stepsize][3];
    printf("%s\n", "Generating test matrices, this may take some time...");
    generate_testset(1234, n, stepsize, tests);
    printf("Running tests for %zu multiplications %zu times: %zu multiplications in total\n", n / stepsize, iterations, n * iterations);
    struct matrix2x2 normalset[n/stepsize];
    struct cmp_matrix2x2 cmpset[n/stepsize];
    for (size_t i = 0; i < n/stepsize; i++) {
    ...
    for (size_t i = 0; i < n/stepsize; i++) {
        results_normal[i] = mulMatrix2x2(normalset[i], normalset[i], multiplicationBignum, additionBignum);
    }
    ...
    struct cmp_matrix2x2 results_cmp[n/stepsize];
    struct timespec start_cmp;
    clock_gettime(CLOCK_MONOTONIC, &start_cmp);
    for (size_t i = 0; i < n/stepsize; i++) {
        results_cmp[i] = mulCmpMatrix2x2(cmpset[i], cmpset[i], multiplicationBignum, additionBignum);
    }
    ...
    for (size_t i = 0; i < n / stepsize; i++) {
        ...
        for (size_t j = 0; j < iterations; j++) {
            free2x2(mulMatrix2x2(normalset[i], normalset[i], multiplicationBignum, additionBignum));
        }
        ...
        clock_gettime(CLOCK_MONOTONIC, &start);
        for (size_t j = 0; j < iterations; j++) {
            freeCmp2x2(mulCmpMatrix2x2(cmpset[i], cmpset[i], multiplicationBignum, additionBignum));
        }
        clock_gettime(CLOCK_MONOTONIC, &end);

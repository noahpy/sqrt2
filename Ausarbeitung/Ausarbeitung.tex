...
Fedora 38 x86\_64, Linux-Kernel 6.3.8 ausgeführt. Kompiliert wurde mit der Option -O1 mit GCC 13.1.1 \par
...
Auch für die Multiplikation gibt es noch effizientere Algorithmen wie den \textit{Toom-Cook Algorithmus}, der im Gegensatz zur Karazuba-Multiplikation nicht zwingend in zwei, sondern beliebig viele Teile aufteilt und somit eine Laufzeit von $\mathcal{O}(n^{1.46})$ erreicht\cite{the_art_of_programming}. \\
Eine andere Möglichkeit wäre Parallälität durch Threading bei der Multiplikation von Bignums oder Matrizen zu nutzen. Ob diese Optimierungen in der Praxis auch schneller sind, müsste gemessen werden, da sie weiteren Overhead verursachen. \par
